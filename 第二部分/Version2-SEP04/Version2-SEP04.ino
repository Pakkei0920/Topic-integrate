...
void loop() {
  ...
  if (buttonState == HIGH && canrecording && situation == 0) {
  ...
  else if (buttonState == LOW && endrecording) {
    canrecording = true;
    delay(500);
  }
  if (situation == 1){
    xTaskCreate(fixi2s_adc, "fixi2s_adc", 1024 * 2, NULL, 1, NULL);
    Serial.println(situation);
    }
...
void uploadFile(){
  ...
  HTTPClient client;
  client.begin("http://topic.paki91.com/upload_audio.php");
